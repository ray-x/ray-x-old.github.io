...
## [银角大王-武沛齐Django-Drf框架与vue项目搭建案例全套教学 完整版~哔哩哔哩bilibili~](https://www.bilibili.com/video/BV1pA41117U4)

## [全站最牛逼的DRF（Django-restframework），没有之一！~哔哩哔哩bilibili~](https://www.bilibili.com/video/BV1XR4y157rk)

## [【独家专授】我要安利给所有人DRF（Django-rest-framework）与源码解析自学教程全程无广安心食用！！！~哔哩哔哩bilibili~](https://www.bilibili.com/video/BV1Cv4y1R77V)
...
1.  First get Auth from setting.py and in each view read
    `authentication_class`. The 2nd will override global setting if it
...
-   It will automatically generate a set of fields for you, based on the
    model.
-   It will automatically generate validators for the serializer, such
    as unique~together~ validators.
...
1.  `read_only` can be used for output serializer it can be shown in
    response
...
2.  It follow conventions of Django ModelForm

3.  Use a new name `gender_info` because when write to DB we want a
...
2.  You can create a new ModelSerializer
...
2.  It also apply when M2N is passed e.g. {\'tags\': \[1, 1111\]}, if
    `1111` not existed in M2N table, validation will fail

### Override `to_presentation`

1.  If you need to show something in DB in a more friendly way (beyond
    `display_xxx`) You can override `to_presentation`

2.  ![](../assets/image_1696746627345_0.png)
